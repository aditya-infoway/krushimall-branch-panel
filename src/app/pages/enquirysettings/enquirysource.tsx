...
const [enquirySources, setEnquirySources] =
  useState<EnquirySource[]>([]);
  ...
  const formValidationRules = {
    enquirySource: { required: "Enquiry source is required" },
  };
const getEnquirySources = async () => {
  try {
...
useEffect(() => {
  ...
  const handleOpenEditDrawer = (item: EnquirySource) => {
    setEditId(item.id);
    reset({
...
const handleDelete = async (id: number) => {
 ...
 const handleBulkDelete = async () => {
  try {
    if (
      ...
      await Promise.all(
        selectedIds.map((id) =>
          apiHelper.delete(`/enquiry-sources/${id}`)
        )
      );
 ...
 const handleToggleStatus = async (id: number) => {
  try {
    await apiHelper.patch(
      `/enquiry-sources/toggle-status/${id}`,
      {}
    );
 ...
 const onFormSubmit = async (
  ...
  // Filter data
  const filteredData = enquirySources.filter((item) => {
    const matchesSearch =
      item.enquirySource.toLowerCase().includes(search.toLowerCase());
  ...
  return (
    <div className="relative min-h-screen space-y-6 p-4 pb-28 text-gray-900 md:p-6 dark:text-gray-100">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="dark:text-dark-300 mt-1 text-sm text-gray-500">
            Manage all enquiry sources from here
          </p>
        </div>
      ...
      <div className="relative w-full max-w-md">
        ...
        <input
          type="text"
          placeholder="Search enquiry source..."
          value={search}
          onChange={(e) => {
              ...
              >
                ...
                <div className="dark:border-dark-500 flex items-center justify-between border-b border-gray-200 px-5 py-4">
                  <h2 className="dark:text-dark-50 text-lg font-semibold text-gray-800">
                    {editId !== null ? "Edit Enquiry Source" : "Add Enquiry Source"}
                  </h2>
                  <Button
