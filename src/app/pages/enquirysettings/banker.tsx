...
const [bankers, setBankers] =
  ...
  const formValidationRules = {
    banker: { required: "Banker name is required" },
  };
...
useEffect(() => {
  ...
  const handleOpenEditDrawer = (item: Banker) => {
    setEditId(item.id);
    reset({
      banker: item.banker,
      status: item.status,
 ...
 const handleDelete = async (id: number) => {
  try {
    await apiHelper.delete(`/bankers/${id}`);
    getBankers();
  } catch (error) {
    console.log(error);
  }
...
const handleToggleStatus = async (id: number) => {
 ...
 const onFormSubmit = async (
  ...
  // Filter data
  const filteredData = bankers.filter((item) => {
    const matchesSearch =
      item.banker.toLowerCase().includes(search.toLowerCase());
              ...
              >
                ...
                <div className="dark:border-dark-500 flex items-center justify-between border-b border-gray-200 px-5 py-4">
                  <h2 className="dark:text-dark-50 text-lg font-semibold text-gray-800">
                    {editId !== null ? "Edit Banker" : "Add Banker"}
                  </h2>
                  <Button
